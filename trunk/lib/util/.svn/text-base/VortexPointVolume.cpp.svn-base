...
bool HasImaginaryRoot(float a0, float a1, float a2, float a3)
{
...
bool IsVortex( const kvs::Matrix33f& J )
{
    // The characteristic equation for J is represented by
    //
    // r^3 + P r^2 + Q r + R = 0
    //
    //   P = -tr(J)
    ...
    const float a3 = 1;
    const float a2 = - J.trace();
    const float a1 =
          J[0][0] * J[1][1] - J[0][1] * J[1][0]
        + J[1][1] * J[2][2] - J[1][2] * J[2][1]
        + J[2][2] * J[0][0] - J[2][0] * J[0][2];
    const float a0 = - J.determinant();

    return HasImaginaryRoot(a0, a1, a2, a3);
}

bool FindVortexPoint( const float* values, kvs::Vector3f* point, kvs::Matrix33f* tensor )
{
    ...
    {
        const kvs::Matrix33f J = VelocityGradientTensor( values, current );
        const kvs::Vector3f V = InterpolatedVector( values, current );
        const kvs::Vector3f diff = -J.inverted() * V;
        if ( diff.length() < 0.001 )
        {
            if ( !IsIncluded( current ) ) return false;
            if ( !IsVortex( J ) ) return false;
...
VortexPointVolume::VortexPointVolume( const kvs::StructuredVolumeObject* volume ):
...
VortexPointVolume::VortexPointVolume( const kvs::StructuredVolumeObject* volume, const float ignore_value )
{
    this->setIgnoreValue( ignore_value );
    this->exec( volume );
}

void VortexPointVolume::setIgnoreValue( const float ignore_value )
{
    m_has_ignore_value = true;
    m_ignore_value = ignore_value;
}

VortexPointVolume::SuperClass* VortexPointVolume::exec( const kvs::ObjectBase* object )
{
        ...
        for ( size_t k = 0; k < dimz - 1; k++ )
        {
            ...
            for ( size_t j = 0; j < dimy - 1; j++ )
            {
                ...
                for ( size_t i = 0; i < dimx - 1; i++ )
                {
                    ...
                    if ( ::FindVortexPoint( data, &local, &tensor ) )
                    {
...
bool VortexPointVolume::include_ignore_value( const float* values ) const
{
