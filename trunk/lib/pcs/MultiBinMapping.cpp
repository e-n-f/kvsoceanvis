 ...
 *
 *  $Id: MultiBinMapping.cpp 326 2012-12-22 06:19:41Z naohisa.sakamoto $
 */
/*****************************************************************************/
#include "MultiBinMapping.h"
...
MultiBinMapping::MultiBinMapping()
{
}

MultiBinMapping::MultiBinMapping( const kvs::ObjectBase* object )
{
    this->exec( object );
}

MultiBinMapping::MultiBinMapping( const kvs::ObjectBase* object, const kvs::ValueArray<kvs::UInt32>& nbins )
{
    SuperClass::m_nbins = nbins;
    this->exec( object );
}

MultiBinMapping::SuperClass* MultiBinMapping::exec( const kvs::ObjectBase* object )
{
    if ( !object )
    {
    ...
    const kvs::TableObject* table = reinterpret_cast<const kvs::TableObject*>( object );
...
//            const kvs::Real64 value = table->readValue( i, j );
            const kvs::Real64 value = table->column(j).at<kvs::Real64>(i);
...
//    SuperClass::m_min_values = table->minValueList();
//    SuperClass::m_max_values = table->maxValueList();
...
//    SuperClass::m_min_ranges = table->minRangeList();
//    SuperClass::m_max_ranges = table->maxRangeList();
...
size_t MultiBinMapping::get_nbins_by_sturges_formula( const kvs::TableObject* table, const size_t index )
{
...
size_t MultiBinMapping::get_nbins_by_scott_choice( const kvs::TableObject* table, const size_t index )
{
    const kvs::AnyValueArray& array = table->column(index);
...
kvs::Real64 MultiBinMapping::get_bin_width_by_scott_choice( const kvs::AnyValueArray& array )
{
    // Scott's choice.
...
kvs::Real64 MultiBinMapping::get_standard_deviation( const kvs::AnyValueArray& array )
{
...
kvs::Real64 MultiBinMapping::get_mean( const kvs::AnyValueArray& array )
{
    const size_t nvalues = array.size();
    kvs::Real64 sum = 0.0;
    for ( size_t i = 0; i < nvalues; i++ )
...
kvs::Real64 MultiBinMapping::get_value( const kvs::AnyValueArray& array, const size_t index )
{
