...
protected:
    ...
    float CalcCoCoef(const kvs::ValueArray<float>& x, const kvs::ValueArray<float>& y);
    kvs::ValueArray<float> GetTabColumn(const kvs::TableObject* input, const int index);
    kvs::ValueArray<float> CalcTabCoCoef(const kvs::TableObject* input);
