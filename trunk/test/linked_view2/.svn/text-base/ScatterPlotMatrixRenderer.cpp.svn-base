 ...
 *
 *  $Id: ScatterPlotMatrixRenderer.cpp 1130 2012-05-05 06:22:06Z s.yamada0808@gmail.com $
 */
/*****************************************************************************/
#include <cassert>
#include <kvs/OpenGL>
...
float ScatterPlotMatrixRenderer::CalcCoCoef(const kvs::ValueArray<float>& x, const kvs::ValueArray<float>& y)
{
    ...
    for (size_t i=0; i<n; i++)
    {
        tmp1 += (x[i] - xavg)*(y[i] - yavg);
        tmp2 += (x[i] - xavg)*(x[i] - xavg);
        tmp3 += (y[i] - yavg)*(y[i] - yavg);
    }
    return (tmp1 / (sqrt(tmp2)*sqrt(tmp3)));
}

kvs::ValueArray<float> ScatterPlotMatrixRenderer::GetTabColumn(const kvs::TableObject* input, const int index)
{
    size_t nr = input -> nrows();
    kvs::ValueArray<float> output(nr);
    for (size_t i=0; i<nr; i++)
    {
        output[i] = input->column(index).to<float>(i);
    }
    return output;
}

kvs::ValueArray<float> ScatterPlotMatrixRenderer::CalcTabCoCoef(const kvs::TableObject* input)
{
    size_t nc = input->ncolumns();
    size_t n = (nc*(nc - 1))/2;
    kvs::ValueArray<float> output(n);
    kvs::ValueArray<float> tmp1;
    kvs::ValueArray<float> tmp2;
    size_t idx = 0;
    for (size_t i=0; i<(nc-1); i++)
    {
        for (size_t j=i+1; j<nc; j++)
        {
            tmp1 = GetTabColumn(input, i);
            tmp2 = GetTabColumn(input, j);
            float tmp = CalcCoCoef(tmp1, tmp2);
            output[idx] = tmp;
            idx += 1;
        }
    }
...
void ScatterPlotMatrixRenderer::exec( kvs::ObjectBase* object, kvs::Camera* camera, kvs::Light* light )
{
    ...
    */
    ...
    kvs::ValueArray<float> corvalues;
    corvalues = CalcTabCoCoef(dynamic_cast<kvs::TableObject*>(object));
    m_color_map.setRange( -1, 1 );

    kvs::Matrix<int> idxlist(ncolumns, ncolumns);
    idxlist.zero();
    int idxtmp = 0;
    ...
    for ( int i = 0; i < ncolumns; i++ )
    {
        for ( int j = 0; j < ncolumns; j++ )
        {
            ...
            const float color_value = corvalues[idxlist[x_index][y_index]];
